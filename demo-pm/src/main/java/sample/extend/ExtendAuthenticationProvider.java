...
// ~ Methods
    ...
    public Authentication authenticate(Authentication authentication)
        ...
        if (user == null) {
            ...
            try {
                user = this.retrieveUser(extendKey, (ExtendAuthenticationToken) authentication);
    ...
    protected UserDetails retrieveUser(String extendKey,
        ...
        ExtendUserDetailsService userDetailsService = this.getExtendUserDetailsService();
        UserDetails loadedUser = userDetailsService.loadUserByExtendKey(authentication);
        if (loadedUser == null) {
            throw new InternalAuthenticationServiceException(
                    "UserDetailsService returned null, which is an interface contract violation");
        }
        if (loadedUser.getAuthorities() == null) {
            throw new InternalAuthenticationServiceException("用户权限为空");
        }
        return loadedUser;
    }
        ...
        public void check(UserDetails user) {
            if (!user.isCredentialsNonExpired()) {
                ...
                throw new CredentialsExpiredException(messages.getMessage(
                        ...
                        "User credentials have expired [" + user.getUsername() + "]"));
            }
        }
    }
